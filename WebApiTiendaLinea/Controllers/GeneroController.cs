...
namespace WebApiTiendaLinea.Controllers
{
    ...
    public class GeneroController : ControllerBase
    {
        [HttpPost]
        [Route("Registrar")]
        public IActionResult RegistrarGenero([FromBody] clsGenero genero)
        {
            try
            {
                bool resultado = GeneroData.Registrar(genero);
                if (resultado)
                {
        ...
        public IActionResult ActualizarGenero([FromBody] clsGenero genero)
        {
            try
            {
                bool resultado = GeneroData.Actualizar(genero);
                if (resultado)
                {
        ...
        public IActionResult EliminarGenero(int id)
        {
            try
            {
                bool resultado = GeneroData.Eliminar(id);
                if (resultado)
                {
        ...
        public IActionResult ListarGeneros()
        {
            try
            {
                List<clsGenero> generos = GeneroData.Listar();
                return Ok(generos);
            }
            catch (Exception ex)
            {
