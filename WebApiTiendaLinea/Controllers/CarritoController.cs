...
namespace WebApiTiendaLinea.Controllers
{
    ...
    public class CarritoController : ControllerBase
    {
        [HttpPost]
        [Route("Registrar")]
        public IActionResult RegistrarCarritoDeCompras([FromBody] clsCarrito carrito)
        {
            try
            {
                bool resultado = CarritosDeCompras.Registrar(carrito);
                if (resultado)
                {
        ...
        public IActionResult ActualizarCarritoDeCompras([FromBody] clsCarrito carrito)
        {
            try
            {
                bool resultado = CarritosDeCompras.Actualizar(carrito);
                if (resultado)
                {
        ...
        public IActionResult EliminarCarritoDeCompras(int id)
        {
            try
            {
                bool resultado = CarritosDeCompras.Eliminar(id);
                if (resultado)
                {
        ...
        public IActionResult ListarCarritosDeCompras()
        {
            try
            {
                List<clsCarrito> carritos = CarritosDeCompras.Listar();
                return Ok(carritos);
            }
            catch (Exception ex)
            {
