...
namespace WebApiTiendaLinea.Controllers
{
    ...
    public class EnvioController : ControllerBase
    {
        [HttpPost]
        [Route("Registrar")]
        public IActionResult RegistrarEnvio([FromBody] clsEnvio2 envio)
        {
            try
            {
                bool resultado = Envio.Registrar(envio);
                if (resultado)
                {
        ...
        public IActionResult ActualizarEnvio([FromBody] clsEnvio envio)
        {
            try
            {
                bool resultado = Envio.Actualizar(envio);
                if (resultado)
                {
        ...
        public IActionResult EliminarEnvio(int id)
        {
            try
            {
                bool resultado = Envio.Eliminar(id);
                if (resultado)
                {
        ...
        public IActionResult ListarEnvios()
        {
            try
            {
                List<clsEnvio> envios = Envio.Listar();
                return Ok(envios);
            }
            catch (Exception ex)
            {
